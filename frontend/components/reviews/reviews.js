...
import { voteOnReview } from '../../js/api.js';
...
export const updateSortButtonCounts = (currentReviews) => {
    ...
    const sortControlsContainer = document.getElementById('review-sort-controls-container'); // Changed ID

    if (!sortPositiveBtn || !sortNegativeBtn) {
        console.warn("Sorting buttons not found during count update.");
        if (sortControlsContainer && sortControlsContainer.classList)
            sortControlsContainer.classList.add('hidden');
        return;
    }
    ...
    if (sortControlsContainer && sortControlsContainer.classList) {
        if (positiveCount + negativeCount > 0) {
            sortControlsContainer.classList.remove('hidden');
        } else {
            sortControlsContainer.classList.add('hidden');
        }
    }
...
export const handleReviewSubmit = async (event, exchangeId) => {
    ...
    const reviewForm = document.getElementById('review-form'); // Get form inside handler
    if (!reviewForm) {
    ...
    const reviewSubmitError = document.getElementById('review-submit-error'); // Get error element
    const reviewSubmitSuccess = document.getElementById('review-submit-success'); // Get success element
    const guestNameInput = document.getElementById('guest-name'); // Get guest name input
    const reviewRatingInputContainer = document.getElementById('review-rating-input-container');


    clearErrorMessage('review-submit-error'); // Assuming this helper is available or define locally
    if (reviewSubmitError) reviewSubmitError.classList.add('hidden'); // Direct hide
    if (reviewSubmitSuccess) reviewSubmitSuccess.classList.add('hidden'); // Direct hide
    ...
    if (!isLoggedIn()) {
        ...
        if (!guestName) {
            ...
            if (reviewSubmitError) reviewSubmitError.classList.remove('hidden'); // Direct show
            submitButton.disabled = false;
            submitButton.textContent = 'Опубликовать';
    ...
    try {
        ...
        const { submitItemReview } = await import('../../js/api.js'); // Dynamically import if not globally available
        await submitItemReview(exchangeId, reviewData);
        console.log('Review submission successful (API call).');
        if (reviewSubmitSuccess) reviewSubmitSuccess.classList.remove('hidden'); // Direct show
        if (reviewSubmitSuccess) reviewSubmitSuccess.textContent = 'Review submitted successfully! It is pending moderation.';
        reviewForm.reset();
    } catch (error) {
